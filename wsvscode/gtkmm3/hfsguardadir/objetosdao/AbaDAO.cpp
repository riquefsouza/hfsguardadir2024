...
int AbaDAO::consultarTotal() {
	...
	strcpy(sSQL, "select count(*) from Abas");
...
std::vector<Aba> AbaDAO::consultarTudo(IProgressoLog progressoLog) {
	...
	char sSQL[TAM_MAX_STR];
	strcpy(sSQL, "select cod, nome from Abas");
...
bool AbaDAO::incluir(Aba obj) {
	...
	int res = 0;
	char sSQL[TAM_MAX_STR];
	strcpy(sSQL, "insert into Abas(cod, nome) values(:cod,:nome)");
...
bool AbaDAO::alterar(Aba obj) {
	...
	int res = 0;
	char sSQL[TAM_MAX_STR];
	strcpy(sSQL, "update Abas set nome=:nome where cod=:cod");
