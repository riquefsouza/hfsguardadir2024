...
void Rotinas::MsgDlg(Glib::ustring sMensagem,
  ...
  Gtk::MessageDialog dialog(sMensagem, false, tipo, Gtk::BUTTONS_OK, true);
...
std::string Rotinas::TrimRight(const std::string texto)
{
	...
	stexto.erase(std::find_if(stexto.rbegin(), stexto.rend(),
		std::not_fn(std::ptr_fun<int, int>(std::isspace))).base(), stexto.end());
...
Glib::ustring Rotinas::formataDate(const Glib::ustring formato, const Glib::DateTime dt) {
	return dt.format(formato);
}

Glib::DateTime Rotinas::StringToDate(const Glib::ustring sData) {
	Glib::DateTime data;
	...
	if (sdh.length() > 0) {
		...
		data = Glib::DateTime::create_local(
...
Glib::DateTime Rotinas::DataHoraNow() {
	return Glib::DateTime::create_now_local();
}

Glib::DateTime Rotinas::RawToDataHora(const time_t rawTempo)
{
	struct tm infoTempo;
	Glib::DateTime data;
...
Arquivo Rotinas::FileInfoToArquivo2(const Glib::ustring sCaminho) {
	...
	if (garquivo) {
		...
		tempoGArquivo = info->get_modification_date_time();
		...
		//gdatahora = g_date_time_new_from_unix_local(&tempoGArquivo);
		//arq.setModificado(DateTime(gdatahora));
		arq.setModificado(tempoGArquivo);
		...
		if (tipo == Gio::FileType::FILE_TYPE_DIRECTORY) {
			atributos += "[DIR]";
		}
		if (tipo == Gio::FileType::FILE_TYPE_REGULAR) {
			atributos += "[ARQ]";
		}
...
Arquivo Rotinas::FileInfoToArquivo(const std::string sNomeArquivo) {
	...
	if (access(sNomeArquivo.c_str(), 0) == 0) {
		...
		arquivo.setModificado(Glib::DateTime::create_now_local());
...
void Rotinas::testar() {
	...
	*/
	...
	for (unsigned int i = 0; i < tamlistaDir; i++)
	{
		...
		std::cout << CharToWChar(saida) << std::endl;
	}



	std::snprintf(saida, TAM_MAX_STR, "sDir: [%s]\n", sDir.c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "AppPath: %s\n", sApp.c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "FileExists: %s\n", (FileExists(sApp) ? "true" : "false")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "DirectoryExists: %s\n", (DirectoryExists(sAppDir) ? "true" : "false")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "ExtractFilePath: %s\n", ExtractFilePath(sApp).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "ExtractFileName: %s\n", ExtractFileName(sApp).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "Length(): %d\n", sDir.length()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "Trim(): [%s]\n", Trim(sDir).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "tamanho Trim(): %d\n", Trim(sDir).length()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "LowerCase(): %s\n", LowerCase(sDir).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "UpperCase(): %s\n", UpperCase(sDir).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "Pos('.'): %d\n", Pos(".", sDir)); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "LastDelimiter('.'): %d\n", LastDelimiter(".", sDir)); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "QuotedStr(): %s\n", QuotedStr(sDir).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "IntToStr(245789): %s\n", IntToStr(245789).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "StrToInt() --> 245789: %d\n", StrToInt("245789")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "StrToLong() --> 4200155136: %llu\n", StrToLong("4200155136")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "DoubleToStr(15.557878): %s\n", DoubleToStr(15.557878).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "SubString(): %s\n", SubString(sDir, 8, 6).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "ReplaceAll(): [%s]\n", ReplaceAll(sDir, "\\", "@").c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "trocaSeparador(): [%s]\n", trocaSeparador(sDir).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "testaNull(NULL): %s\n", testaNull(NULL).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "StartsStr(DVD, JOGOS2_DVDS/TESTE): %s\n", (StartsStr("DVD", "JOGOS2_DVDS/TESTE") ? "true" : "false")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "StartsStr(DVD, DVDSJOGOS2_/TESTE): %s\n", (StartsStr("DVD", "DVDSJOGOS2_/TESTE") ? "true" : "false")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "ContainsStr(JOGOS2_DVDS/TESTE): %s\n", (ContainsStr("DVDSJOGOS2_/TESTE", "DVDSJOGOS2_/TESTE") ? "true" : "false")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "EndsStr(TESTE, JOGOS2_DVDS/TESTE): %s\n", (EndsStr("TESTE", "DVDSJOGOS2_/TESTE") ? "true" : "false")); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "formataDate(): %s\n", formataDate(FORMATO_DATAHORA, DataHoraNow()).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	Glib::DateTime dt = StringToDate("03/02/2017 15:23:32");
	std::snprintf(saida, TAM_MAX_STR, "StringToDate(03/02/2017 15:23:32): %s\n", formataDate(FORMATO_DATAHORA, dt).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "FormatLong(4200155136): %s\n", ULongToStr(4200155136).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "FormatDouble(15.544878): %s\n", DoubleToStr(15.544878).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "FormatDouble(15.547878): %s\n", DoubleToStr(15.547878).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "FormatDouble(15.557878): %s\n", DoubleToStr(15.557878).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "FormatDouble(15.567878): %s\n", DoubleToStr(15.567878).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));

	std::snprintf(saida, TAM_MAX_STR, "MontaTamanho() --> 3.91 GB: %s\n", MontaTamanho(4200155136).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "MontaTamanho() --> 3.50 GB: %s\n", MontaTamanho(3757244416).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "MontaTamanho() --> 2.29 GB: %s\n", MontaTamanho(2455207936).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "MontaTamanho() --> 888.18 PB: %s\n", MontaTamanhoBig(999999999999999999.9999).c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	...
	{
		linha = lista.at(i);
		std::snprintf(saida, TAM_MAX_STR, "SListSeparador[%d]: %s\n", i, linha.c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	}

	std::string arquivo = ExtractFilePath(sApp);
	arquivo += "\\exemplo.txt";

	std::snprintf(saida, TAM_MAX_STR, "arquivo: %s\n", arquivo.c_str()); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
	...
	if (SListSaveToFile(lista, arquivo)) {
		...
		if (FileExists(arquivo)) {
			...
			if (DeleteFromFile(arquivo)) {
				if (!FileExists(arquivo)) {
					std::snprintf(saida, TAM_MAX_STR, "DeleteFromFile: OK\n"); std::cout << CharToWChar(saida) << std::endl; //OutputDebugString(CharToWChar(saida));
				}
			}
			else {
