...
Catalogador::Catalogador() {
	...
	frmPrincipal->get_window()->set_cursor(Gdk::Cursor::create(Gdk::WATCH));
	...
	frmPrincipal->get_window()->set_cursor(Gdk::Cursor::create(Gdk::ARROW));
...
int Catalogador::verificaNomeDiretorio(const Glib::ustring sCaminho, RotuloRaiz *sRotuloRaiz) {
	...
	if (sRotuloRaiz->getRotulo().length() == 0) {
        ...
        if (Rotinas::Trim(sRotuloRaiz->getRotulo()).length() > 0) {
