...
FrmCadExtensao::FrmCadExtensao()
	...
	colunaCadExtIcone = manage(new class Gtk::TreeViewColumn("Ícone", modeloTabelaExtensao.coluna1));
	colunaCadExtIcone->set_min_width(100);
	colunaCadExtIcone->set_title("Ícone");
	
	colunaCadExtExtensao = manage(new class Gtk::TreeViewColumn("Extensão", modeloTabelaExtensao.coluna0));
	colunaCadExtExtensao->set_min_width(150);
	colunaCadExtExtensao->set_title("Extensão");
	...
	menuExtrairIconesArquivos->set_label("Extrair Ícones dos Arquivos");
	...
	menuImportarIconesArquivos->set_label("Importar Ícones dos Arquivos");
	...
	menuExportarIcone->set_label("Exportar para Ícone");
	...
	menuExcluirTodasExtensoes->set_label("Excluir Todas Extensões");
	...
	menuExcluirExtensao->set_label("Excluir Extensão");
	...
	menuIncluirExtensao->set_label("Incluir Extensão");
	...
	menuExtensao->set_label("Extensão");
	...
	this->set_title("Cadastro de Extensão de Arquivo");
...
void FrmCadExtensao::on_menuExcluirExtensao_activate(){
	...
	if (catalogador->listaExtensoes.size() > 0) {
		...
		if (res) {
			...
			path.get_from_selection_data(data, path);
			//Glib::ArrayHandle<int> aindices = path.get_from_selection_data(data, path);
			//path.get_indices();
			...
			Dialogo::mensagemInfo(frmCadExtensao, data.get_text().c_str());
