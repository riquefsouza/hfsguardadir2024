  ...
  begin
    case s[i] of
      ...
      begin
        ...
        if s[i]=formatsettings.decimalseparator then decimalfound:=true
...
begin
  ...
  begin
    ...
    begin
      ...
      {$ELSE}
             Result := '0'+formatsettings.decimalseparator+'0 E 0';
      {$IFEND}
      exit;
    end
    else  Result := '';
    ...
    {$ELSE}
    Result := Result+FormatSettings.decimalseparator+s;
  ...
  begin {normal}
    ...
    begin
      ...
      begin
        ...
        {$ELSE}
         insert(FormatSettings.decimalseparator, s, exponent+1)
         {$IFEND}
        else
        begin
          i := length(s);
      ...
      {$ELSE}
       for i := 1 to -exponent do
          s := '0'+s;
        s := '0'+FormatSettings.decimalseparator+s;
      end;
      Result := ss+s;
      n      := pos(FormatSettings.decimalseparator, Result);
      {$IFEND}
...
begin
  ...
  {$ELSE}
  n := pos(FormatSettings.decimalseparator, s);
  {$IFEND}
...
//  TempLog2R.Reciprocal(localsig);
  ...
  {$ELSE}
  TempLog2R.Assign('1'+FormatSettings.decimalseparator+'44269504089', MaxSig);
  Tempexp512R := TBigFloat.Create(MaxSig);
  Tempexp512R.Assign('1'+FormatSettings.decimalseparator+'953125e-3', MaxSig);
  {$IFEND}
