...
#include "../objetosdao/VisaoDAO.h"
#include "../objetosgui/Dialogo.h"
#include "../objetosgui/EscolhaArquivo.h"
...
void Catalogador_Pesquisar() {
	...
	if (gtk_tree_model_iter_n_children(arvore_modelo, NULL) > 0) {
		...
		GtkPaned* splitPesquisa = GTK_PANED(FrmPrincipal.spPesquisa);
...
void Catalogador_SetCursor(GdkCursorType cursorType) {
	GdkScreen* screen = gdk_screen_get_default();
	GdkWindow* win = gdk_screen_get_root_window(screen);
	GdkCursor* cursor = gdk_cursor_new_for_display(gdk_display_get_default(), cursorType);
