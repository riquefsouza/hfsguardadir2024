...
GString* EscolhaArquivo_escolher(GtkWidget *pai, const gchar *filtro, const GString *titulo,
	...
	GtkWidget *dialog = gtk_file_chooser_dialog_new(Rotinas_retUTF8(titulo->str),
		GTK_WINDOW(pai),
		acao,
	...
	gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(dialog), arquivo->str);
	gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dialog), diretorio->str);
	gtk_file_chooser_set_do_overwrite_confirmation(GTK_FILE_CHOOSER(dialog), TRUE);

	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
	{
		if (acao == GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER)
			sretorno = g_string_new(gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog)));
		else
			sretorno = g_string_new(gtk_file_chooser_get_current_folder(GTK_FILE_CHOOSER(dialog)));
	}
