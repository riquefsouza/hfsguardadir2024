...
unsigned int Rotinas::calculaProgresso(int nMaximo, int nProgresso) {
	nMaximo += 1;
	nProgresso += 1;

	unsigned int valor = nProgresso / nMaximo;
...
ImagemBuffer Rotinas::BlobToPixbuf(const void* blob, size_t tamanho) {
	...
	catch (std::exception& e)
	{
		...
		std::cerr << "BlobToPixbuf Erro: " << e.what() << std::endl;
	}
...
size_t Rotinas::PixbufToBlob(ImagemBuffer pixbuf, void** blob, const std::string &formato) {
	...
	catch (std::exception& e)
	{
		...
		std::cerr << "PixbufToBlob Erro: " << e.what() << std::endl;
	}
...
void Rotinas::testar() {
	...
	*/
	...
	std::snprintf(saida, TAM_MAX_STR, "Pos('.'): %d\n", (int)Pos(".", sDir)); std::cout << saida << std::endl; //OutputDebugString(CharToWChar(saida));
	std::snprintf(saida, TAM_MAX_STR, "LastDelimiter('.'): %d\n", (int)LastDelimiter(".", sDir)); std::cout << saida << std::endl; //OutputDebugString(CharToWChar(saida));
