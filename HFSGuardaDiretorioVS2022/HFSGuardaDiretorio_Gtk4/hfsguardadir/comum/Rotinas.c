...
GdkTexture* Rotinas_XPMToTexture(const char** xpm)
{
	GdkPixbuf* pxb = gdk_pixbuf_new_from_xpm_data(xpm);
	GdkTexture* txtr = gdk_texture_new_for_pixbuf(pxb);
	g_object_unref(pxb);

	return txtr;
}

GdkTexture* Rotinas_imagemToTexture(const char* sNomeImagem) {
	GString* caminho = Rotinas_AppDirPath();
	caminho = g_string_append(caminho, "imagens/");
	caminho = g_string_append(caminho, sNomeImagem);

	GError* erro = NULL;
	GFile* file = g_file_new_for_path(caminho->str);
	GdkTexture* texture = gdk_texture_new_from_file(file, &erro);
	g_object_unref(file);
...
GdkPixbuf* Rotinas_extrairPixbufArquivo(const GString* sCaminho, int tamanho) {
	...
	if (garquivo != NULL) {
		...
		imagem = GTK_IMAGE(gtk_image_new_from_gicon(icone));
...
size_t Rotinas_LastDelimiter(const GString* sub, const GString* texto) {
...
gchar* Rotinas_cNewStr(size_t tamanho)
{
	gchar *retorno = NULL;
...
size_t Rotinas_cLastDelimiter(const gchar* sub, const gchar* texto)
{
	gchar* parte;
...
GString* Rotinas_ExtractFilePathWindows(const GString *sCaminho)
{
	...
	DWORD dwRet = GetCurrentDirectory(tamanho, Rotinas_charArrayToLPCWSTR(caminho));
...
{
	DWORD atributos = GetFileAttributes(Rotinas_charArrayToLPCWSTR(sNomeArquivo->str));
...
gboolean Rotinas_DeleteFromFileWindows(const GString *sNomeArquivo) {
	return DeleteFile(Rotinas_charArrayToLPCWSTR(sNomeArquivo->str));
}
...
const gchar** Rotinas_criarArrayDeString(const gchar* texto) {
	int numStrings = 1;
...
void Rotinas_testar() {
	...
	if (pixbuf != NULL) {
		OutputDebugString(Rotinas_charArrayToLPCWSTR("Rotinas_extrairPixbufArquivo OK"));

		if (Rotinas_PixbufToArquivo(pixbuf, "C:\\Temp\\blob\\dir32.bmp", Rotinas_EXTENSAO_BMP)) {
			OutputDebugString(Rotinas_charArrayToLPCWSTR("Rotinas_PixbufToArquivo OK"));
		}
	}

	pixbuf = Rotinas_BlobToPixbuf(blob, ntamanho);
	if (pixbuf != NULL){
		OutputDebugString(Rotinas_charArrayToLPCWSTR("Rotinas_BlobToPixbuf OK"));

		blob = NULL;
		gsize ntamanho2 = Rotinas_PixbufToBlob(pixbuf, &blob, Rotinas_EXTENSAO_BMP);
		if (ntamanho2 > 0 && blob != NULL) {
			OutputDebugString(Rotinas_charArrayToLPCWSTR("Rotinas_PixbufToBlob OK"));
		}
	...
	if (pixbuf != NULL) {
		OutputDebugString(Rotinas_charArrayToLPCWSTR("Rotinas_imagemToPixbuf OK"));
	...
	if (pixbuf != NULL) {
		OutputDebugString(Rotinas_charArrayToLPCWSTR("Rotinas_ArquivoToPixbuf OK"));
	...
	*/
	...
	g_string_printf(saida, "sDir: [%s]\n", sDir->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "AppDirPath: %s\n", sApp->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "FileExists: %s\n", (Rotinas_FileExists(sApp) ? "true" : "false")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "DirectoryExistsLinux: %s\n", (Rotinas_DirectoryExistsLinux(sApp) ? "true" : "false")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "ExtractFilePath: %s\n", Rotinas_ExtractFilePath(sApp)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "ExtractFileName: %s\n", Rotinas_ExtractFileName(sApp)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "Length(): %d\n", sDir->len); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	GString *sTrim = Rotinas_Trim(sDir);
	g_string_printf(saida, "Trim(): [%s]\n", sTrim->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "tamanho Trim(): %d\n", sTrim->len); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "LowerCase(): %s\n", Rotinas_LowerCase(sDir)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "UpperCase(): %s\n", Rotinas_UpperCase(sDir)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "Pos('.'): %d\n", Rotinas_Pos(g_string_new("."), sDir)); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "LastDelimiter('.'): %d\n", Rotinas_LastDelimiter(g_string_new("."), sDir)); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "QuotedStr(): %s\n", Rotinas_cQuotedStr(sDir->str)); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "IntToStr(245789): %s\n", Rotinas_IntToStr(245789)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "StrToInt() --> 245789: %d\n", Rotinas_StrToInt(g_string_new("245789"))); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "StrToLong() --> 4200155136: %llu\n", Rotinas_StrToLong(g_string_new("4200155136"))); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "DoubleToStr(15.557878): %s\n", Rotinas_DoubleToStr(15.557878)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "SubString(): %s\n", Rotinas_SubString(sDir, 8, 6)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "cStringReplaceAll(): [%s]\n", Rotinas_cReplaceAll(sDir->str, "\\", "@")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "trocaSeparador(): [%s]\n", Rotinas_trocaSeparador(sDir)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "testaNull(NULL): %s\n", Rotinas_testaNull(NULL)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "StartsStr(DVD, JOGOS2_DVDS/TESTE): %s\n", (Rotinas_StartsStr(g_string_new("DVD"), g_string_new("JOGOS2_DVDS/TESTE")) ? "true" : "false")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "StartsStr(DVD, DVDSJOGOS2_/TESTE): %s\n", (Rotinas_StartsStr(g_string_new("DVD"), g_string_new("DVDSJOGOS2_/TESTE")) ? "true" : "false")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "ContainsStr(JOGOS2_DVDS/TESTE): %s\n", (Rotinas_ContainsStr(g_string_new("DVDSJOGOS2_/TESTE"), g_string_new("DVDSJOGOS2_/TESTE")) ? "true" : "false")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "EndsStr(TESTE, JOGOS2_DVDS/TESTE): %s\n", (Rotinas_EndsStr(g_string_new("TESTE"), g_string_new("DVDSJOGOS2_/TESTE")) ? "true" : "false")); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "formataDate(): %s\n", Rotinas_formataDate(Rotinas_FORMATO_DATAHORA, g_date_time_new_now_local())->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	GDateTime* dt = Rotinas_StringToDate(g_string_new("03/02/2017 15:23:32"));		
	g_string_printf(saida, "StringToDate(03/02/2017 15:23:32): %s\n", g_date_time_format(dt, Rotinas_FORMATO_DATAHORA)); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "FormatLong(4200155136): %s\n", Rotinas_ULongToStr(4200155136)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "FormatDouble(15.544878): %s\n", Rotinas_DoubleToStr(15.544878)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "FormatDouble(15.547878): %s\n", Rotinas_DoubleToStr(15.547878)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "FormatDouble(15.557878): %s\n", Rotinas_DoubleToStr(15.557878)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "FormatDouble(15.567878): %s\n", Rotinas_DoubleToStr(15.567878)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));

	g_string_printf(saida, "MontaTamanho() --> 3.91 GB: %s\n", Rotinas_MontaTamanho(4200155136)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "MontaTamanho() --> 3.50 GB: %s\n", Rotinas_MontaTamanho(3757244416)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "MontaTamanho() --> 2.29 GB: %s\n", Rotinas_MontaTamanho(2455207936)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
	g_string_printf(saida, "MontaTamanho() --> 888.18 PB: %s\n", Rotinas_MontaTamanhoBig(999999999999999999.9999)->str); OutputDebugString(Rotinas_charArrayToLPCWSTR(saida->str));
